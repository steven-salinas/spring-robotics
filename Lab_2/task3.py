'''
	task3.py completes manuever of moving around two circles of radius R1
	and connecting lines with distances D1. Resulting shape should be in
	the form of a running track.
'''
import utils
import time
...
# Stop robot by pressing Cntrl + C
...
# Beginning of main function for task3.py
if __name__ == "__main__":
	...
	if X<(distance/Y):
		...
		# Turn in radius R1 at velocity X
		utils.turnRV(R1,X)
		...
		# Second turn in radius R1 at velocity X
		utils.turnRV(R1,X)
		while abs(headingError)>resolution:
			angle=utils.getIMUDegrees()
			headingError= (angle - endHeading + 540)%360 - 180
